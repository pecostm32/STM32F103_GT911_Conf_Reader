...
//  while((GPIOA->IDR & 0x0001) == 0);
  ...
  int16_t c;
  uint8_t state   = STATE_WAIT_INPUT;
  uint8_t mode    = COMMAND_MODE_NONE;
  uint8_t *target = 0;
  ...
  while(1)
  {
    ...
    if(c != -1)
    {
      ...
      switch(state)
      {
        case STATE_WAIT_INPUT:
          //Check if a Set or Get instruction received
          if(c == 'S')
          {
            state = STATE_WAIT_COMMAND;
            mode  = COMMAND_MODE_SET;
          }
          else if(c == 'G')
          {
            state = STATE_WAIT_COMMAND;
            mode  = COMMAND_MODE_GET;
          }
        ...
        case STATE_WAIT_COMMAND:
          //Check on start of command received
          switch(c)
          {
            default:
              //Reset state machine on erroneous input
              state = STATE_WAIT_INPUT;
              mode  = COMMAND_MODE_NONE;
              
              //On not recognized character an error 1 is returned
              SendString("E1\n");
              break;
        ...
        case STATE_WAIT_D:
          switch(c)
          {
            default:
              ...
              //On not recognized character after D an error 2 is returned
              SendString("E2\n");
              break;
              
            case 'C':
              //Start of DCO commands
              state = STATE_WAIT_DCO;
              break;

            case 'R':
              //Dynamics Routing needs to be handled based on mode
              if(mode == COMMAND_MODE_SET)
              {
                //Start of Dynamics Routing command
                state = STATE_WAIT_DYNAMICS_ROUTING;
              }
              else if(mode == COMMAND_MODE_GET)
              {
                //On a get return the current dynamics routing
                SendDynamicsRouting();
            ...
            case 'W':
              //Start of Depth Wheel commands
              state = STATE_WAIT_DEPTH_WHEEL;
              break;
          }
          break;
        ...
        case STATE_WAIT_DCO:
          ...
          else
          {
             ...
             //On not recognized character after DC an error 3 is returned
             SendString("E3\n");
          }
          break;
          
        case STATE_WAIT_DCO_LETTER:
          if(c == 'A')
          {
    ...
    //Hier een soort state machine maken die de binnen komende data verwerkt.
    //Eerst dus een lijst van mogelijke commando's maken.
    
    //xxx => 0 - 255
    //t => S for set or G for get
    //Command is terminated with CR
    
    //Master module only settings
    //tMT=xxx        Master Tune.
    //tPW=xxx        Pitch Wheel value.
    //tDW=xxx        Depth wheel value.
    //tPWR=r         Pitch wheel routing.  r => A for DCO A, B for DCO B or AB for both
    //tDWR=r         Depth wheel routing.  r => 1 for LFO I-II, 3 for LFO III or 13 for both
    //tLFOyD=xxx     LFO Depth.            y => 1 or 3
    //tLFOyS=xxx     LFO Speed.            y => 1 or 3
    //tLFOyR=r       LFO Routing.          y => 1 or 3  r => none for no routing, A for DCO A, B for DCO B, F for Filter (only when y = 3) Combination of parms for multiple routing
    //tLFO3W=w       LFO III waveform.     w => T for Triangle, Q for sQuare or TQ for both
    //tNL=xxx        Noise Level.
    //tPWy=xxx       Pulse Width for DCO.  y => A or B
    //tDCOBDy=xxx    DCO B detune.         y => F for fine or C for coarse
    //tDCOyF=z       DCO footage.          y => A or B  z => 16, 8, 4
    
    //Voice module settings
    //tMV=xxx        Master Volume
    //tEGA=xxx       Envelope Generator Attack
    //tEGD=xxx       Envelope Generator Decay
    //tEGS=xxx       Envelope Generator Sustain
    //tEGR=xxx       Envelope Generator Release    
    //tFC=xxx        Filter Cutoff
    //tFR=xxx        Filter Resonance
    //tFA=xxx        Filter ADSR amount
    //tFK=y          Filter Keyboard tracking              y => 0 or 1
    //tDCOyW=w       DCO Waveform                          y => A or B  w => none for no waveform, S for Sawtooth, Q for sQuare or SQ for both
    //tDCOBH=h       DCO B Half volume                     h => 0 or 1
    //tEGOR=r        Envelope Generator Output Routing     r => none for no routing, F for Filter, A for Amplifier, FA for both
    //tDR=r          Dynamics Routing                      r => none for no routing, L for EG level, T for Attack Time, LT for both
    //tVnK=xxx       Voice Key value                       n => 1 to 6 for specific voice
    //tVnD=xxx       Voice Dynamics value                  n => 1 to 6 for specific voice
    //tVnG=g         Voice Gate value (Key pressed)        g => 0 or 1
...
void SendDynamicsRouting()
{
  SendString("Dynamics Routing: ");
  
  if(voiceparameters.DynamicsToAttackTime)
    usbSend('T');
    
  if(voiceparameters.DynamicsToEnvelopeLevel)
    usbSend('L');
...
void dmach3IrqHandler(void)
{
  ...
  //Output in range of +/-400, so in total less then 10 bits signed data
  //In the voice modules this can be added to the voice signal
  //Each voice signal can consist of 4 waves. So in total 5 signals of +/-400 added together can result in a max of +/-2000, which is slightly less then the 12 bits of the DAC
  noise = (int32_t)(randz) % 400;
  
  //The noise signal needs to have a minimal amount always present to allow the filters to self oscillate.
  //So master noise level needs to be > 0 at minimum and 1 at maximum.
